...
<!-- Mirrored from template.hasthemes.com/reid/reid/portfolio-details.php by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 24 May 2023 09:11:12 GMT -->
<head>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <title>Reid - جزئیات نمونه کارها</title>
...
function insertComments($comment, $full_name , $email , $website){
    ...
    $sql = "INSERT `$table_comment` SET `comment`= ? , `full_name` = ? , `email` = ? , `website` = ? ";
    $result = $connection->prepare($sql);
    $result->bindValue(1,$comment);
    $result->bindValue(2,$full_name);
    $result->bindValue(3,$email);
    $result->bindValue(4,$website);
    $result->execute();
    return $result ;
}
//RUN
...
if (isset($_GET["submit"])){
    if (!empty($_GET["comment"]) and !empty($_GET["full_name"]) and !empty($_GET["email"]) and filter_var($_GET["email"] , FILTER_VALIDATE_EMAIL) and !empty($_GET["website"])) {
        ...
        elseif (!filter_var($_GET["email"] , FILTER_VALIDATE_EMAIL)){
            $hasError = true ;
            $errors[] = "فرمت ایمیل صحیح نمیباشد" ;
        }
